...
# Headless plotting for Seraph compute nodes.
import matplotlib
matplotlib.use("Agg")
...
def get_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=int,
        default=25,
        help="Refresh training_reward.png every N training episodes; 0 disables periodic refresh",
    )
    p.add_argument(
        "--reward_ma_window",
        type=int,
        default=50,
        help="Moving-average window used in the training reward plot",
    )
    return p.parse_args()
...
def _read_training_reward(train_csv: Path) -> tuple[np.ndarray, np.ndarray]:
    """Read normalized episode reward from train_log.csv.

    The plotted reward is ``avg_qoe_per_user_slot``.  This is preferred to an
    unnormalized episode sum because all comparisons should remain interpretable
    even if the evaluation horizon is changed in a later experiment.
    """
    episodes: list[int] = []
    rewards: list[float] = []
    ...
    with train_csv.open("r", newline="", encoding="utf-8") as f:
        for row in csv.DictReader(f):
            try:
                ep = int(row["episode"])
                reward = float(row["avg_qoe_per_user_slot"])
...
def plot_training_reward(
    ...
    """Save/refresh the PPO training reward convergence plot.

    The file is written atomically so opening it while a long job is running
    does not expose a half-written PNG.
    """
    episodes, rewards = _read_training_reward(train_csv)
    if rewards.size == 0:
        return
    ...
    ma_x, ma_y = _moving_average(episodes, rewards, int(ma_window))
    if ma_y.size:
        ax.plot(
            ma_x,
            ma_y,
            linewidth=2.0,
            label=f"Moving average ({int(ma_window)})",
        )

    ax.set_xlabel("Training episode")
    ax.set_ylabel("Average QoE reward / user-slot")
    ax.set_title(f"RSU-only PPO training reward (seed={int(seed)})")
    ax.grid(True, alpha=0.25)
    ax.legend()
    fig.tight_layout()

    output_png.parent.mkdir(parents=True, exist_ok=True)
    tmp_png = output_png.with_name(output_png.stem + ".tmp.png")
    fig.savefig(tmp_png, dpi=180, bbox_inches="tight")
    plt.close(fig)
    os.replace(tmp_png, output_png)


def safe_plot_training_reward(
    train_csv: Path,
    output_png: Path,
    ma_window: int,
    seed: int,
) -> None:
    """Plotting must not destroy an otherwise valid long training job."""
    try:
        ...
        print(f"[PLOT-WARN] failed to update reward plot: {type(exc).__name__}: {exc}")
...
def main() -> None:
    ...
    if args.reward_ma_window <= 0:
        raise ValueError("--reward_ma_window must be positive")
    ...
    if not args.eval_only:
        ...
        reward_plot = out_dir / "training_reward.png"
        fields = [
            "episode",
        ...
        with train_csv.open("w", newline="", encoding="utf-8") as f:
            ...
            for ep in range(args.episodes):
                ...
                if (
                    args.plot_every > 0
                    and ((ep + 1) % args.plot_every == 0 or ep == args.episodes - 1)
                ):
                    safe_plot_training_reward(
                        train_csv=train_csv,
                        output_png=reward_plot,
        ...
        # Always refresh once after the training CSV is closed, even when
        # periodic plotting is disabled.
        safe_plot_training_reward(
            train_csv=train_csv,
            output_png=reward_plot,
            ma_window=args.reward_ma_window,
