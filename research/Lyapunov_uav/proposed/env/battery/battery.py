...
class UAVBattery:
    ...
    def __init__(self, config: BatteryConfig):
        ...
        self.round_horizon = max(1, config.target_service_slots_per_round)

    def reset_episode(self):
        self.soc = float(self.config.e_init)
        self.virtual_q = 0.0
        self.round_start_soc = self.soc
        self.round_horizon = max(1, self.config.target_service_slots_per_round)
    ...
    def start_round(self, round_horizon: int):
